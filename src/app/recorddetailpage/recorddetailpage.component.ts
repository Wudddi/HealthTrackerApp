...
@Component({
  ...
  templateUrl: './recorddetailpage.component.html',
  styleUrl: './recorddetailpage.component.css'
})
export class RecorddetailpageComponent {
  ...
  constructor() {
    ...
    this.recordDal.select(id).then(data=>{
      this.record = data
      if (this.record.location) {
        this.lat = this.record.location.latitude;
        this.lon = this.record.location.longitude;
        this.showMap();
      }
    }).catch((e)=>{
      console.log(e)
  ...
  onUpdateClick() {
    this.recordDal.update(this.record)
      .then((data) => {
        console.log(data);
